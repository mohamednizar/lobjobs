			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					...
					if(self.attr("afterUpload")=="link" || self.attr("afterUpload")=="filename" || self.attr("afterUpload")=="image" || self.attr("afterUpload")=="none")
					{
						...
						var xx = responseJSON.path + responseJSON.filename.replace("'", "%27");
		
						
						
						var image_path="<div id='rnd' style='display:inline-block'><img id='img_rnd' src="+xx+" width='100px' height='100px'></div>";
						//$("#mug_container").prepend(image_path);
...
var image = new fabric.Image(imgObj);
    ...
    containment: "#mug_container",
     ...
     /* $.ajax({
					...
					//alert(data);
					//$(artist).empty();//empty the list first
					//$(artist).append(data);//add ot the list
...
alert(JSON.stringify(canvas));
	 /*canvas1 = new fabric.Canvas('c2');
	canvas1.loadFromJSON(JSON.stringify(canvas));
	 canvas.renderAll(); */  
