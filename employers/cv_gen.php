...
$ac = ("SELECT DATEDIFF(end_date,now()) as period,end_date,start_date,type FROM account_control WHERE id='$org' and  id  IN (SELECT id FROM org_info where track=1 )  ");
        ...
        $type = $info2['type'];
        $period =$info2['period'];
         echo '<span  style="position:relative;left:7%;top:30px;font-size:1.5em;">From:'.$sdate.'  To:'.$edate.' Remaining  :  '.$period.'  Days </span>';
      $periodInt = (int)$period;
      
      if($periodInt<= 0){
      redirect('activation.php?id='.$orgid);
      }
        }
...
if (isset($_GET['org']) && isset($_GET['cvview'])){
                    	$orgid = $_GET['org'];
			$today = date("Y-m-d");
    ...
    if (isset($_GET['org'])) {
        ?>
                        ...
                        $org= $_GET['org'];
                            ...
                            <div class='col-md-3 h1_head_sub'><a href='cv_gen.php?download=yes&id=<?php echo $id ?>&org=<?php echo $org ?>' type="submit" id="approve" class="btn btn-success" target="_blank">Download</a><a type="button" class="btn btn-info" onclick="head_toggle()">Foward</a>
                    </div>
                            
                            <?php
                        } else {
                            ?>
                            <div class='col-md-3 h1_head_sub'>><a href='cv_gen.php?download=yes&id=<?php echo $id ?>&org=<?php echo $org ?>' type="submit" id="approve" class="btn btn-success" target="_blank">Download</a><a type="button" class="btn btn-default btn-lg" onclick="head_toggle()">Foward</a>
                    </div>
    ...
    ?>
                        ...
                        if (isset($_GET['id'])) {
                            if (!$cv =="" ) {
                                    ...
                                    ?>
                                    <div class='col-md-3 h1_head_sub'><a class="btn btn-success" id="dld" href="<?php echo "../jobseeker/files/" . $cv  ?>" target="_blank">Original Cv</a></div><div class='col-md-9'></div>
                                    <?php
    ...
    <script type="text/javascript" >
        ...
        var idofurl = getParameterByName("id", pathname);//get the id
            ...
            function getConfirmation(){
               ...
               if( retVal == true ){
               alert('ok');
               $link = "cv_gen.php?delete=yes&id=" + idofurl;
               window.location.href = $link;
               $link2 = "jobseekers_cv.php?delete=yes&id=" + idofurl;
        ...
        var pathname = document.URL;
        var idofurl = getParameterByName("org", pathname);//get the id
        
         $.ajax({
                type: "post",
                url: "update_dldcount.php",
                data: {
                    org: idofurl
                },
                success: function (data) {
